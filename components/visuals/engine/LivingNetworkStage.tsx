...
export function LivingNetworkStage() {
  ...
  return (
    <div className="relative mx-auto w-full overflow-hidden rounded-none bg-[#F2EAE0] p-6 text-charcoal sm:p-100_30px_70px_-15px_rgba(0,0,0,0.85)] border border-vaultAmber/20">
      ...
      <div className="relative z-10 mt-8 grid gap-8 lg:grid-cols-12 lg:items-center">
        ...
        <div className="relative flex min-h-[420px] w-full flex-col justify-between rounded-3xl border border-vaultAmber/20 bg-voidSurface p-6 lg:col-span-6">
          <div className="flex items-center justify-between border-b border-vaultAmber/10 pb-3 font-mono text-[10px] text-deepink/50 uppercase">
            <span>LIVING NEURAL MAP</span>
            <span className="text-mark font-medium">SELECT NODE TO TRAVEL</span>
          </div>
          ...
          <div className="relative my-4 flex h-[300px] w-full items-center justify-center">
              ...
              {NETWORK_NODES.map((node) => {
                ...
                return (
                  <motion.line
                    ...
                    x2={node.x}
                    y2={node.y}
                    stroke={isSelected ? node.color : "#9B8EC7"}
            ...
            {/* Central Core Label */}
            <div className="absolute top-[46%] left-[50%] -translate-x-1/2 -translate-y-1/2 rounded-full border border-vaultAmber/30 bg-mist px-3 py-1 font-mono text-[9px] font-bold text-vaultAmber0_0_15px_rgba(157,142,199,0.3)] pointer-events-none">
              LAXVISH NUCLEUS
            </div>
            ...
            {NETWORK_NODES.map((node) => {
              ...
              return (
                <button
                  ...
                  className={`group absolute -translate-x-1/2 -translate-y-1/2 p-2.5 font-mono text-left transition-colors duration-200 cursor-pointer ${ isSelected ? "border-2 bg-mist text-deepink z-20" : "border border-rule-hair bg-cream text-deepink/75 hover:border-mark hover:text-mark z-10" }`}
                >
          ...
          <div className="border-t border-vaultAmber/10 pt-3 font-mono text-xs">
            ...
            <p className="mt-1 text-[11px] text-charcoal/70 font-mono">
        ...
        <div className="flex flex-col space-y-4 lg:col-span-6">
          <div className="rounded-3xl border border-vaultAmber/20 bg-voidSurface p-2 ">
          ...
          {/* Context & Link */}
          <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-vaultAmber/15 bg-mist/30 p-4 font-mono text-xs ">
            <div>
              <span className="block text-[10px] uppercase text-mark font-medium">
            ...
            <Link
              ...
              className="inline-flex items-center gap-1.5 rounded-full border border-mark bg-mark px-4 py-1.5 font-mono text-[11px] font-medium text-cream transition-colors hover:bg-deepink "
            >
