...
export function Footer() {
  ...
  return (
    <footer className="relative z-10 border-t border-vaultAmber/20 bg-[#050507]">
      <div className="mx-auto w-full max-w-[96rem] px-6 py-24 sm:px-12 lg:px-16">
        <div className="grid gap-16 lg:grid-cols-[1.2fr_2.5fr]">
          <div>
            <p className="text-sm font-semibold tracking-[0.25em] uppercase text-vaultAmber font-mono">
              Laxvish
            </p>
          ...
          <div className="grid gap-12 sm:grid-cols-2 md:grid-cols-4">
            {FOOTER_GROUPS.map((group) => (
              <div key={group.title}>
                <h3 className="text-sm font-semibold tracking-wide text-vaultAmber font-mono">
                  {group.title}
                </h3>
                <ul className="mt-6 space-y-4 font-mono text-xs">
                  {group.links.map((link) => (
                    <li key={link.href}>
                      <Link
                        href={link.href}
                        className="text-charcoal/70 transition-colors duration-300 hover:text-vaultAmber"
                      >
                        {link.label}
                      </Link>
                    </li>
        ...
        <div className="mt-24 flex flex-col gap-6 border-t border-vaultAmber/15 pt-8 text-xs font-medium text-charcoal/60 sm:flex-row sm:items-center sm:justify-between font-mono">
          ...
          <div className="flex items-center gap-6">
            {SOCIAL_LINKS.map((item) => (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="transition-colors duration-300 hover:text-vaultAmber"
              >
                {item.label}
              </a>
            ))}
