...
const faqItems: FaqItem[] = [
  ...
  },
  {
    ...
    answer:
      "ChatGPT is a general tool: it is great for exploration but not built for your business. Laxvish is built around your workflows, your data, and your rules. The AI worker is trained on your business, knows your tone, follows your policies, and integrates with the tools you already use.",
  },
];
...
export function FaqAccordion() {
  return (
    ...
    >
      ...
      <div className="mt-8 sm:mt-10 border-t border-charcoal/20">
        {faqItems.map((item, index) => (
          ...
          >
            <summary className="flex min-h-[48px] cursor-pointer items-center justify-between py-4.5 sm:py-6 text-base sm:text-lg lg:text-xl font-normal text-charcoal outline-none marker:hidden select-none focus-visible:ring-2 focus-visible:ring-charcoal">
              <span className="pr-4">{item.question}</span>
              <span className="text-xl font-light text-neonCyan transition-transform duration-300 group-open:rotate-45 shrink-0">
                +
              </span>
            </summary>
            <p className="pb-5 sm:pb-6 max-w-3xl text-sm sm:text-base leading-relaxed text-charcoal/70">
              {item.answer}
            </p>
          </details>
        ))}
